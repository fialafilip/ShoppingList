...
function FamilyManager({ onClose }) {
 ...
 const createFamily = useMutation({
   mutationFn: async (name) => {
     const { data } = await axios.post('http://localhost:5000/api/family', 
       { name },
       { withCredentials: true }
     );
     return data;
 ...
 const removeMember = useMutation({
   mutationFn: async ({ familyId, userId }) => {
     await axios.delete(
       `http://localhost:5000/api/family/${familyId}/members/${userId}`,
       { withCredentials: true }
     );
   },
   onSuccess: () => {
               ...
               <div className="flex justify-between items-center mb-4">
                 ...
                 <button
                   onClick={() => setSelectedFamily(
                     selectedFamily?._id === family._id ? null : family
                   )}
