...
const isAdmin = async (req, res, next) => {
  try {
    ...
    const member = family.members.find((m) => m.userId.toString() === req.user._id.toString());
...
router.post('/:familyId/members', isAuthenticated, isAdmin, async (req, res) => {
  try {
    ...
    if (isAlreadyMember) {
      return res.status(400).json({ message: 'Uživatel je již členem rodiny' });
    }
...
// Změnit roli člena
router.patch('/:familyId/members/:userId/role', isAuthenticated, isAdmin, async (req, res) => {
  try {
    const member = req.family.members.find((m) => m.userId.toString() === req.params.userId);
    ...
    const adminCount = req.family.members.filter((m) => m.role === 'admin').length;
...
// Odstranit člena
router.delete('/:familyId/members/:userId', isAuthenticated, isAdmin, async (req, res) => {
  try {
